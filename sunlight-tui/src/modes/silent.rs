...
pub fn render_silent(fb: &mut Framebuffer, layout: &Layout, state: &SilentModeState) {
    ...
    font::draw_str(fb, pct_x + font::text_width(pct_str, 1), pct_y, "%", palette::TEXT_DIM, 1);
    ...
    for (i, detail) in state.details.iter().enumerate() {
        if let Some(msg) = detail {
            ...
            let symbol_color = if is_last { palette::ACCENT } else { palette::SUCCESS };
            ...
            if is_last && state.progress < 1000 {
                ...
                draw::spinner_frame(fb, spinner_x + 8, y + 8, 6, 2, state.spinner_step, 108, palette::ACCENT);
            }
        }
    }
}
