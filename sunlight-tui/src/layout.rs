...
impl Layout {
    pub fn new(fb_width: u32, fb_height: u32) -> Self {
        ...
        let main_h = footer_y.saturating_sub(main_y).saturating_sub(SEPARATOR_THICKNESS);
        ...
        Self { fb_width, fb_height, header, main, footer }
    }
    ...
    pub fn draw_chrome(&self, fb: &mut Framebuffer) {
        ...
        // Header background
        fb.fill_rect(self.header.x, self.header.y, self.header.w, self.header.h, palette::SURFACE);
        
        // Footer background
        fb.fill_rect(self.footer.x, self.footer.y, self.footer.w, self.footer.h, palette::SURFACE);
        
        // Separator below header
        fb.hline(0, self.header.h, self.fb_width, palette::SEPARATOR);
        
        // Separator above footer
        fb.hline(0, self.footer.y - SEPARATOR_THICKNESS, self.fb_width, palette::SEPARATOR);
    }

    /// Clear the main zone to background color
    pub fn clear_main(&self, fb: &mut Framebuffer) {
        fb.fill_rect(self.main.x, self.main.y, self.main.w, self.main.h, palette::BG);
    }
}
