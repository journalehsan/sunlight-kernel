...
fn encode_manifest(out: &mut Vec<u8>, m: &SourceManifest) -> Result<(), IndexError> {
    ...
    put_u64(out, m.source_id.get());
    put_u64(out, m.root_id);
    put_u8(out, m.scope.as_u8());
    put_u64(out, m.owner);
    put_string(out, &m.relative_path, 1024)?;
    put_u64(out, m.canonical_path_hash);
    match m.file_identity {
        Some(v) => {
            put_u8(out, 1);
            put_u64(out, v.device);
            put_u64(out, v.inode);
        }
        None => put_u8(out, 0),
    }
    ...
    put_u8(out, m.needs_digest_upgrade as u8);
    put_u64(out, m.size_bytes);
    put_opt_u64(out, m.modified_at_ns);
    for v in [
        m.parser_id,
        m.parser_version,
        m.tokenizer_id,
        ...
        m.chunking_id,
        m.chunking_version,
        m.ignore_config_version,
    ] {
        put_u32(out, v);
    }
    put_u64(out, m.indexed_at_ns);
    put_u8(out, m.state.as_u8());
    put_u32(out, m.chunk_count);
    put_opt_u64(out, m.document_memory_id);
    put_u32(out, m.source_revision);
    put_u16(out, m.missing_confirmations);
    match &m.failure {
        Some(v) => {
            put_u8(out, 1);
...
fn decode_manifest(r: &mut Reader<'_>) -> Result<SourceManifest, IndexError> {
    ...
    let source_id =
        SourceId::from_raw(r.u64()?).map_err(|_| IndexError::InvalidValue("source id"))?;
    let root_id = r.u64()?;
    let scope = MemoryScope::from_u8(r.u8()?).ok_or(IndexError::InvalidValue("scope"))?;
    let owner = r.u64()?;
    let relative_path = r.string(1024)?;
    let canonical_path_hash = r.u64()?;
    let file_identity = match r.u8()? {
        0 => None,
        1 => Some(FileIdentity {
            device: r.u64()?,
            inode: r.u64()?,
        }),
        _ => return Err(IndexError::InvalidValue("file identity flag")),
    };
    let content_digest = if r.peek_digest_version()? == 0 {
        r.unset_digest()?
    } else {
        ContentDigest::decode(r.take(35)?)?
    };
    let fast_fingerprint = r.opt_u64()?.map(FastFingerprint::new);
    let legacy_content_hash = r.opt_u64()?.map(LegacyFnvContentHash::new);
    let needs_digest_upgrade = r.bool()?;
    let size_bytes = r.u64()?;
    let modified_at_ns = r.opt_u64()?;
    let parser_id = r.u32()?;
    let parser_version = r.u32()?;
    let tokenizer_id = r.u32()?;
    let tokenizer_version = r.u32()?;
    let chunking_id = r.u32()?;
    let chunking_version = r.u32()?;
    let ignore_config_version = r.u32()?;
    let indexed_at_ns = r.u64()?;
    let state = SourceState::from_u8(r.u8()?).ok_or(IndexError::InvalidValue("source state"))?;
    let chunk_count = r.u32()?;
    let document_memory_id = r.opt_u64()?;
    let source_revision = r.u32()?;
    let missing_confirmations = r.u16()?;
    let failure = match r.u8()? {
        0 => None,
        1 => Some(SourceFailure {
            kind: SourceFailureKind::from_u8(r.u8()?)
                .ok_or(IndexError::InvalidValue("failure kind"))?,
    ...
    let pending_import = match r.u8()? {
        0 => None,
        1 => Some(decode_pending(r)?),
        _ => return Err(IndexError::InvalidValue("pending flag")),
    };
    Ok(SourceManifest {
        manifest_version,
        source_id,
...
fn encode_pending(out: &mut Vec<u8>, p: &PendingImport) {
    put_u16(out, p.format_version);
    out.extend_from_slice(&p.import_key.encode_canonical());
    put_u64(out, p.source_id.get());
    put_u32(out, p.expected_revision);
    out.extend_from_slice(&p.content_digest.encode());
    let v = p.pipeline_versions;
    for n in [
        v.parser_id,
        v.parser_version,
        v.tokenizer_id,
        ...
        v.chunking_id,
        v.chunking_version,
        v.ignore_config_version,
    ] {
        put_u32(out, n);
    }
    put_u8(out, p.state as u8);
    put_u64(out, p.created_at);
    put_u64(out, p.latest_attempt_at);
    put_u16(out, p.attempt_count);
}
fn decode_pending(r: &mut Reader<'_>) -> Result<PendingImport, IndexError> {
    let format_version = r.u16()?;
    if format_version != 1 {
        return Err(IndexError::InvalidValue("pending version"));
    }
    let import_key =
        ImportKey::decode_canonical(r.take(crate::import_key::IMPORT_KEY_ENCODED_LEN)?)?;
    let source_id =
        SourceId::from_raw(r.u64()?).map_err(|_| IndexError::InvalidValue("pending source"))?;
    let expected_revision = r.u32()?;
    let content_digest = ContentDigest::decode(r.take(35)?)?;
    let pipeline_versions = PipelineVersions {
        parser_id: r.u32()?,
        parser_version: r.u32()?,
        tokenizer_id: r.u32()?,
        tokenizer_version: r.u32()?,
        chunking_id: r.u32()?,
        chunking_version: r.u32()?,
        ignore_config_version: r.u32()?,
    };
    let state =
        PendingImportState::from_u8(r.u8()?).ok_or(IndexError::InvalidValue("pending state"))?;
    Ok(PendingImport {
        format_version,
        import_key,
        source_id,
...
fn put_string(out: &mut Vec<u8>, v: &str, max: usize) -> Result<(), IndexError> {
    if v.len() > max || v.len() > u16::MAX as usize {
        return Err(IndexError::InvalidValue("state string"));
    }
...
impl<'a> Reader<'a> {
    ...
    fn take(&mut self, n: usize) -> Result<&'a [u8], IndexError> {
        let end = self
            .pos
            .checked_add(n)
            .ok_or(IndexError::InvalidValue("state overflow"))?;
        let s = self
            .data
            .get(self.pos..end)
            .ok_or(IndexError::InvalidValue("state truncated"))?;
        self.pos = end;
        Ok(s)
    }
    ...
    fn bool(&mut self) -> Result<bool, IndexError> {
        match self.u8()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(IndexError::InvalidValue("state bool")),
        }
    }
    fn opt_u64(&mut self) -> Result<Option<u64>, IndexError> {
        match self.u8()? {
            0 => Ok(None),
            1 => Ok(Some(self.u64()?)),
            _ => Err(IndexError::InvalidValue("state option")),
        }
    }
    fn string(&mut self, max: usize) -> Result<String, IndexError> {
        let n = self.u16()? as usize;
        if n > max {
            return Err(IndexError::InvalidValue("state string length"));
        }
        core::str::from_utf8(self.take(n)?)
            .map(|s| s.to_string())
            .map_err(|_| IndexError::InvalidValue("state utf8"))
    }
    fn done(&self) -> bool {
        self.pos == self.data.len()
    }
    fn peek_digest_version(&self) -> Result<u16, IndexError> {
        let b = self
            .data
            .get(self.pos + 1..self.pos + 3)
            .ok_or(IndexError::InvalidValue("digest truncated"))?;
        Ok(u16::from_le_bytes(b.try_into().unwrap()))
    }
    fn unset_digest(&mut self) -> Result<ContentDigest, IndexError> {
        let raw = self.take(35)?;
        if raw[0] != 1 || raw[1..3] != [0, 0] || raw[3..] != [0u8; 32] {
            return Err(IndexError::InvalidValue("unset digest"));
        }
        Ok(ContentDigest::unset())
    }
}
...
mod tests {
    ...
    #[test]
    fn state_roundtrip_and_checksum() {
