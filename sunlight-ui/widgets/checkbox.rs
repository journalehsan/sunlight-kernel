...
impl<'a> Checkbox<'a> {
    pub const fn new(rect: Rect, label: &'a str) -> Self {
        Self { rect, label, checked: false, active: false }
    }

    pub fn draw(&self, canvas: &mut Canvas, theme: &Theme) {
        let box_rect = Rect::new(self.rect.x, self.rect.y + 2, 14, 14);
        canvas.fill_rect(box_rect, if self.active { theme.panel_alt } else { theme.panel });
        canvas.draw_rect(box_rect, if self.active { theme.accent } else { theme.border });
        ...
        let text_rect = Rect::new(self.rect.x + 20, self.rect.y, self.rect.w.saturating_sub(20), self.rect.h);
