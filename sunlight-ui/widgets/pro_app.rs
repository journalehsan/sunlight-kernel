...
const DOC_HOST_RADIUS: u32 = 20;
const DOC_PAGE_RADIUS: u32 = 10;
...
pub struct DocumentCanvasHost<'a> {
    pub rect: Rect,
    pub title: &'a str,
    pub subtitle: &'a str,
    pub placeholder_badge: &'a str,
    pub footer_note: &'a str,
    pub title_font: Option<&'a dyn VecText>,
    pub subtitle_font: Option<&'a dyn VecText>,
    pub badge_font: Option<&'a dyn VecText>,
    pub footer_font: Option<&'a dyn VecText>,
}

impl<'a> DocumentCanvasHost<'a> {
    ...
    pub fn canvas_rect(&self) -> Rect {
        self.page_rect().inset(24)
    }

    pub fn draw_placeholder(&self, canvas: &mut Canvas, theme: &Theme) {
        fill_vertical_gradient(canvas, self.rect, theme.bg.lighten(14), theme.bg.darken(8));

        let host = self.host_rect();
        canvas.fill_rounded_rect(host, DOC_HOST_RADIUS, theme.panel.darken(8));
        canvas.stroke_rounded_rect(host, DOC_HOST_RADIUS, 1, theme.border);
        ...
        canvas.fill_rounded_rect(page, DOC_PAGE_RADIUS, Color::rgb(0xFB, 0xFA, 0xF7));
        canvas.stroke_rounded_rect(page, DOC_PAGE_RADIUS, 1, Color::rgb(0xD7, 0xD3, 0xCD));
        ...
        canvas.fill_rounded_rect(canvas_rect, 12, Color::rgb(0xFF, 0xFF, 0xFF));
        canvas.stroke_rounded_rect(canvas_rect, 12, 1, Color::rgb(0xE0, 0xDB, 0xD4));
        canvas.hbar(
            canvas_rect.x + 1,
            canvas_rect.y + 1,
            canvas_rect.w - 2,
        ...
        for idx in 0..18 {
            let y = canvas_rect.y + 56 + idx * 26;
            if y + 1 >= canvas_rect.bottom() - 32 {
                break;
            }
            let line_w = canvas_rect.w as i32 - 92 - ((idx % 3) * 38);
            canvas.fill_rect(
                Rect::new(canvas_rect.x + 30, y, line_w.max(120) as u32, 2),
                Color::rgb(0xEB, 0xE6, 0xDF),
            );
        }

        let badge_w = measure_text_width(self.badge_font, self.placeholder_badge) + 28;
        let badge = Rect::new(
            canvas_rect.x + ((canvas_rect.w as i32 - badge_w as i32) / 2),
            canvas_rect.y + canvas_rect.h as i32 / 2 - 18,
            badge_w,
            36,
        );
