...
fn lineage_rejects_sequence_generation_kind_hash_and_truncation() {
    ...
    assert_eq!(
        LineageRecord::decode(&wrong_sequence),
        Err(IdentityError::InvalidSequence)
    );
    let mut wrong_generation = encoded;
    wrong_generation[52..60].copy_from_slice(&2u64.to_le_bytes());
    assert_eq!(
        LineageRecord::decode(&wrong_generation),
        Err(IdentityError::InvalidContinuityGeneration)
    );
    let mut invalid_kind = encoded;
    invalid_kind[92] = 9;
    assert_eq!(
        LineageRecord::decode(&invalid_kind),
        Err(IdentityError::InvalidEventKind)
    );
    let mut corrupt_hash = encoded;
    corrupt_hash[LINEAGE_RECORD_LEN - 1] ^= 1;
    assert_eq!(
        LineageRecord::decode(&corrupt_hash),
        Err(IdentityError::HashMismatch)
    );
    assert_eq!(
        LineageRecord::decode(&encoded[..LINEAGE_RECORD_LEN - 1]),
        Err(IdentityError::InvalidLength)
    );
}
...
fn set_rejects_wrong_identity_and_head_hash() {
    ...
    let other_record =
        LineageRecord::genesis(identity(0x33), event(0x44), GenesisEventKind::Created);
    let other_head = LineageHead::from_record(&other_record);
    assert_eq!(
        validate_identity_set(&root, &other_record, &other_head),
        Err(IdentityError::IdentityMismatch)
    );

    let other_same_identity =
        LineageRecord::genesis(identity(0x11), event(0x55), GenesisEventKind::Created);
    ...
    assert_eq!(
        LineageHead::decode(&encoded[..LINEAGE_HEAD_LEN - 1]),
        Err(IdentityError::InvalidLength)
    );
    let mut corrupt = encoded;
    corrupt[70] ^= 1;
    assert_eq!(
        LineageHead::decode(&corrupt),
        Err(IdentityError::ChecksumMismatch)
    );
}
