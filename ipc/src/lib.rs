...
pub mod TzMsg {
    /// Reply:
    /// - word(0): year<<48 | month<<40 | day<<32 | hour<<24 | minute<<16 | second<<8 | weekday_iso
    /// - word(1): signed UTC offset seconds
    /// - word(2): DST active
    /// - word(3): timezone abbreviation bytes
    ///
    /// The weekday occupies word(0)'s low byte because the register IPC ABI
    /// transports four words. Monday=1 through Sunday=7.
