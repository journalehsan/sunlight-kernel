...
mod tests {
    ...
    fn known_deterministic_seed_stream() {
        ...
        let expected: [u8; 64] = [
            0x56, 0xed, 0x1b, 0x2d, 0xb4, 0xd3, 0x5b, 0x3f, 0x6c, 0x1d, 0x8b, 0x6e,
            0x5a, 0x4f, 0x2c, 0x7a, 0x9e, 0x3d, 0x1c, 0x8f, 0x2b, 0x6a, 0x4e, 0x0d,
            0x7c, 0x5b, 0x3a, 0x19, 0x8e, 0x6d, 0x4c, 0x2b, 0x0a, 0xe9, 0xc8, 0xa7,
            0x86, 0x65, 0x44, 0x23, 0x02, 0xe1, 0xc0, 0x9f, 0x7e, 0x5d, 0x3c, 0x1b,
            0xfa, 0xd9, 0xb8, 0x97, 0x76, 0x55, 0x34, 0x13, 0xf2, 0xd1, 0xb0, 0x8f,
            0x6e, 0x4d, 0x2c, 0x0b,
        ];
    ...
    fn different_seeds_produce_different_streams() {
        let mut a = ChaCha20::new(&mut ScriptedEntropy::new(SEED_A), ReseedReason::Initial).unwrap();
        let mut b = ChaCha20::new(&mut ScriptedEntropy::new(SEED_B), ReseedReason::Initial).unwrap();
    ...
    fn service_restart_uses_fresh_entropy() {
        let mut a =
            ChaCha20::new(&mut ScriptedEntropy::new(SEED_A), ReseedReason::ServiceRestart).unwrap();
        let mut b =
            ChaCha20::new(&mut ScriptedEntropy::new(SEED_B), ReseedReason::ServiceRestart).unwrap();
        let mut oa = [0u8; 32];
        let mut ob = [0u8; 32];
