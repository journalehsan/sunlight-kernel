...
impl VirtioGpu {
    ...
    pub unsafe fn resource_attach_backing(
        ...
        *cmd_v = VirtioGpuResourceAttachBacking {
        ...
        (self.cmd_virt as *mut u8)
            .add(512)
            .write_bytes(0, resp_len as usize);
    ...
    pub fn cursor_pixels_virt(&self) -> u64 {
        self.cursor_virt
    }
    ...
    pub fn cursor_pages_phys(&self) -> [VirtioGpuMemEntry; 4] {
        [
            VirtioGpuMemEntry {
                addr: self.cursor_phys,
                length: 4096,
                padding: 0,
            },
            VirtioGpuMemEntry {
                addr: self.cursor_phys + 4096,
                length: 4096,
                padding: 0,
            },
            VirtioGpuMemEntry {
                addr: self.cursor_phys + 4096 * 2,
                length: 4096,
                padding: 0,
            },
            VirtioGpuMemEntry {
                addr: self.cursor_phys + 4096 * 3,
                length: 4096,
                padding: 0,
            },
        ]
    ...
    unsafe fn submit2(
        ...
        *descs.add(0) = VirtqDesc {
            addr: d0_phys,
            len: d0_len,
            flags: DESC_F_NEXT,
            next: 1,
        };
        *descs.add(1) = VirtqDesc {
            addr: d1_phys,
            len: d1_len,
            flags: DESC_F_WRITE,
            next: 0,
