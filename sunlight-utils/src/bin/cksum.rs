//! Native POSIX-oriented `cksum` command.

#![no_std]
#![no_main]

use sunlight_libc::{exit, write_all, MAX_ARGS, STDERR};
use sunlight_utils::cksum::{self, NativeIo};

#[cfg(not(test))]
#[panic_handler]
fn panic(_info: &core::panic::PanicInfo) -> ! {
    let _ = write_all(STDERR, b"cksum: panic\n");
    exit(101);
}
...
pub extern "C" fn _start(argc: u64, argv: *const *const u8) -> ! {
    ...
    let mut io = NativeIo;
    let status = cksum::run(cksum::user_args(&args[..count]), &mut io);
    exit(status as u64);
}
