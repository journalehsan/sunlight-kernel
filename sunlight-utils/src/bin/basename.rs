//! Native POSIX-oriented `basename` command.

#![no_std]
#![no_main]

use sunlight_libc::{exit, write_all, MAX_ARGS, STDERR, STDOUT};
use sunlight_utils::{basename, native};

#[cfg(not(test))]
#[panic_handler]
fn panic(_info: &core::panic::PanicInfo) -> ! {
    let _ = write_all(STDERR, b"basename: panic\n");
    exit(101);
}
