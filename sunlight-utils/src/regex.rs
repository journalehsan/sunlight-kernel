...
impl Regex {
    pub fn compile(pat: &[u8], icase: bool, whole_line: bool) -> Result<Self, Error> {
        ...
        if whole_line {
            ...
            if !has_bol {
                np.prog[np.plen] = Insn::BOL;
                np.plen += 1;
            }
            let copy_end = if has_eol {
                p.plen.saturating_sub(1)
            } else {
                p.plen
            };
            for i in 0..copy_end {
                np.prog[np.plen] = p.prog[i];
                np.plen += 1;
            }
            if !has_eol {
                np.prog[np.plen] = Insn::EOL;
                np.plen += 1;
            }
            np.prog[np.plen] = Insn::Done;
            np.plen += 1;
            return Ok(Regex {
                prog: np.prog,
                plen: np.plen,
        ...
        loop {
            ...
            match self.prog[pc] {
                Insn::Char(c) => {
                    ...
                    let sb = s[pos];
                    if self.icase {
                        if sb.to_ascii_lowercase() != c.to_ascii_lowercase() {
                            return false;
                        }
                    } else {
                        if sb != c {
                ...
                Insn::BRef(n) => {
                    ...
                    for k in 0..blen {
                        ...
                        let rb = r[k];
                        if self.icase {
                            if sb.to_ascii_lowercase() != rb.to_ascii_lowercase() {
                                return false;
                            }
                        } else {
                            if sb != rb {
...
impl<'a> Compiler<'a> {
    fn emit(&mut self, i: Insn) {
        if self.plen < MAX_PROG {
            self.prog[self.plen] = i;
            self.plen += 1;
        }
    }
    ...
    fn piece(&mut self) -> Result<(), Error> {
        ...
        match self.peek() {
            Some(b'*') => {
                ...
                for i in 0..atom_len {
                    self.prog[self.plen] = atom_buf[i];
                    self.plen += 1;
                }
            ...
            Some(b'[') => {
                ...
                loop {
                    match self.peek() {
                        ...
                        Some(b'[')
                            if self.pos + 2 < self.pat.len() && self.pat[self.pos + 1] == b':' =>
                        {
                            self.pos += 2;
                            let ns = self.pos;
...
mod tests {
    ...
    #[test]
    fn lit() {
        chk(b"hello", b"hello world", true);
        chk(b"hello", b"hi", false);
    }
    #[test]
    fn dot() {
        chk(b"h.llo", b"hello", true);
        chk(b"h.llo", b"h\nllo", false);
    }
    #[test]
    fn bol() {
        chk(b"^hello", b"hello world", true);
        chk(b"^hello", b"say hello", false);
    }
    #[test]
    fn eol() {
        chk(b"world$", b"hello world", true);
        chk(b"world$", b"world x", false);
    }
    #[test]
    fn star() {
        chk(b"ab*c", b"ac", true);
        chk(b"ab*c", b"abbbbc", true);
    }
    #[test]
    fn bracket() {
        chk(b"[aeiou]", b"hello", true);
        chk(b"[a-z]", b"z", true);
        chk(b"[^a-z]", b"5", true);
    }
    #[test]
    fn cclass() {
        chk(b"[[:digit:]]", b"a1b", true);
        chk(b"[[:alpha:]]", b"123", false);
    }
    #[test]
    fn alt() {
        chk(b"cat\\|dog", b"I have a cat", true);
        chk(b"cat\\|dog", b"I have a dog", true);
        chk(b"cat\\|dog", b"fish", false);
    }
    #[test]
    fn icase_() {
        let re = Regex::compile(b"Hello", true, false).unwrap();
        assert!(re.is_match(b"hello"));
    }
    #[test]
    fn whole() {
        let re = Regex::compile(b"abc", false, true).unwrap();
        assert!(re.is_match(b"abc"));
        assert!(!re.is_match(b"abcd"));
    }
    #[test]
    fn grouping() {
        chk(b"\\(ab\\)*", b"abab", true);
    }
    #[test]
    fn backref_() {
        chk(b"\\(..\\)\\1", b"abab", true);
        chk(b"\\(..\\)\\1", b"abac", false);
    }

    #[test]
    fn empty_pat() {
        assert!(Regex::compile(b"", false, false).is_err());
    }
    #[test]
    fn unmatched_bracket() {
        assert!(Regex::compile(b"[abc", false, false).is_err());
    }
}
