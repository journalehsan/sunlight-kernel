...
pub fn parse(bytes: &[u8]) -> Result<SvgDocument, String> {
    if bytes.len() > MAX_SVG_SOURCE_BYTES { return Err(String::from("SVG source exceeds limit")); }
    let options = resvg::usvg::Options::default();
    let tree = resvg::usvg::Tree::from_data(bytes, &options).map_err(|_| String::from("invalid SVG"))?;
    let size = tree.size().to_int_size();
    let width = size.width().min(MAX_SVG_DIMENSION);
    let height = size.height().min(MAX_SVG_DIMENSION);
    if width == 0 || height == 0 { return Err(String::from("SVG has zero dimensions")); }
    Ok(SvgDocument { intrinsic_width: width, intrinsic_height: height, view_box: format!("{}x{}", tree.size().width(), tree.size().height()), source: bytes.to_vec() })
}
...
pub fn rasterize(document: &SvgDocument, key: SvgRasterKey) -> Result<RasterImage, String> {
    ...
    let pixels = (width as usize).checked_mul(height as usize).ok_or_else(|| String::from("SVG pixel overflow"))?;
    if pixels > MAX_SVG_RASTER_PIXELS { return Err(String::from("SVG raster exceeds limit")); }
    let tree = resvg::usvg::Tree::from_data(&document.source, &resvg::usvg::Options::default()).map_err(|_| String::from("invalid SVG"))?;
    let mut pixmap = resvg::tiny_skia::Pixmap::new(width, height).ok_or_else(|| String::from("SVG pixmap allocation failed"))?;
    ...
    for rgba in pixmap.data().chunks_exact(4) {
        ...
        let unpm = |channel: u8| if a == 0 { 0 } else { ((channel as u32 * 255 + a / 2) / a).min(255) };
        argb.push((a << 24) | (unpm(rgba[0]) << 16) | (unpm(rgba[1]) << 8) | unpm(rgba[2]));
    }
    Ok(RasterImage { width, height, pixels: argb })
}
