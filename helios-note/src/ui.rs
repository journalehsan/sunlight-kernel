...
fn draw_header(f: &mut Frame, filename: &str, buffer: &TextBuffer, area: Rect) {
    ...
    let header_text = Line::from(vec![
        Span::styled(" Helios Note ", Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  {} {}", filename, modified_str), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
    ]);
...
fn draw_viewport(f: &mut Frame, buffer: &TextBuffer, cursor: &Cursor, area: Rect) {
    ...
    for i in 0..height {
        ...
        if line_idx < buffer.len() {
            ...
            if let Some(content) = buffer.get_line(line_idx) {
                ...
                spans.push(Span::styled(visible_chars, Style::default().fg(Color::White)));
            }

            lines.push(Line::from(spans));
        } else {
            let tilde_str = format!("{:>width$} │ ~", "~", width = line_num_width);
            lines.push(Line::from(Span::styled(tilde_str, Style::default().fg(Color::DarkGray))));
        }
    }
    ...
    let screen_x = area.x + (line_num_width as u16) + 3 + (visual_col.saturating_sub(cursor.scroll_x) as u16);
...
fn draw_status_bar(
    ...
    if show_search {
        let search_line = Line::from(vec![
            ...
            Span::styled(format!(" {}_", search_input), Style::default().fg(Color::White).bg(Color::Black)),
    ...
    let status_line = Line::from(vec![
        ...
        Span::styled(pos_str, Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD)),
...
fn draw_shortcuts(f: &mut Frame, area: Rect) {
    let shortcuts = Line::from(vec![
        Span::styled(" ^S", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw(" Save "),
        Span::styled(" ^F", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw(" Find "),
        Span::styled(" F3", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw(" Next "),
        Span::styled(" ^Z", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw(" Undo "),
        Span::styled(" ^Y", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw(" Redo "),
...
fn draw_help_modal(f: &mut Frame, area: Rect) {
    ...
    let help_text = vec![
        Line::from(Span::styled(" Helios Note Help ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))),
        Line::from(""),
        ...
        Line::from(""),
        Line::from(Span::styled(" Press Esc or ^G to close help ", Style::default().fg(Color::Green))),
    ];
...
fn draw_quit_modal(f: &mut Frame, area: Rect) {
    ...
    let content = vec![
        Line::from(Span::styled(" Save Unsaved Changes? ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))),
        Line::from(""),
        Line::from(" You have modified changes in this document."),
        Line::from(""),
        Line::from(vec![
            Span::styled("  Y / S ", Style::default().fg(Color::Black).bg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::raw(" Save & Quit   "),
            Span::styled(" D / N ", Style::default().fg(Color::Black).bg(Color::Red).add_modifier(Modifier::BOLD)),
            Span::raw(" Discard   "),
            Span::styled(" Esc / C ", Style::default().fg(Color::Black).bg(Color::White).add_modifier(Modifier::BOLD)),
            Span::raw(" Cancel "),
        ]),
    ];
